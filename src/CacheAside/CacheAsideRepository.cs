    ...
    public class CacheAsideRepository<T> where T : class, IEntity
    {
        ...
        public T GetById(int id)
        {
            ...
            try
            {
                ...
                if (cacheItem != null)
                {
                    Console.WriteLine(string.Format("Entity {0} fetched from cache", id));
            ...
            if (!cacheException)
            {
                try
                {
                    ...
                    if (entity != null)
                    {
                        ...
                        Console.WriteLine(string.Format("Entity {0} loaded in to cache", id));
                    }
                }
                catch (Exception)
                {
        ...
        public void Update(T entity)
        {
            ...
            Console.WriteLine(string.Format("Entity {0} was removed form the cache", entity.Id));
        }

        private string GetCacheKey(int objectId)
        {
            return string.Format("{0}_{1}", typeof(T).FullName, objectId);
        }
    }
}